...
public class YellowDisc : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        if (setCourse)
        {
            ...
            if (transform.position == nextPos)
            {
                ...
                if (completed)
                {
                    nextPos = Player2.spot;
                    setCourse = true;
                    if (transform.position == nextPos)
                    {
                        setCourse = false;
                        Player2.spot -= Vector3.forward;
                    }
                }
            }
        }
    ...
    private void OnMouseDown()
    {
        ...
        if (!hasLeftHome && numberOnDie == 6 && Player2.turn && gameManager.canMove)
        {
            StartMove();
        }
        else if (hasLeftHome && Player2.turn && gameManager.canMove)
        {
            Move(numberOnDie);
        }
        else
    ...
    private void StartMove()
    {
        ...
        lane = gameManager.startLanes[Player2.playerId];
    ...
    public void Move(int numberOnDie)
    {
        if (moveForward)
        {
            if (lane == gameManager.startLanes[Player2.playerId] + 1)
            {
                destination = true;
                if (lanePoint + 1 - numberOnDie < 0)
                {
                    if (Player2.discsLeftHome == 1)
                    {
    ...
    private void MoveForward(int numberOnDie)
    {
        ...
        for (int i = 0; i < numberOnDie; i++)
        {
            ...
            if (notInMiddle)
            {
                ...
                {
                    if (gameManager.tracks[lane, lanePoint].parent.name != "YellowMiddleTrack")
                    {
                        lane--;
                        lanePoint++;
                    }
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        {
            float offset = 0.5f;
            Vector3 pos = other.GetComponent<YellowDisc>().nextPos;
            switch (other.name)
            {
                case "Disc":
